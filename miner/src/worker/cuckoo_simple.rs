...
extern "C" {
    pub fn c_solve(output: *mut u32, nonce: *mut u64, input: *const u8) -> u32;
}
...
impl CuckooSimple {
    ...
    #[inline]
    fn solve(&mut self, pow_hash: &H256) -> usize {
        unsafe {
            let mut nonce = 0u64;
            let mut output = vec![0u32; CYCLE_LEN+1];
            let ns = c_solve(output.as_mut_ptr(), &mut nonce, pow_hash[..].as_ptr());
        ...
        loop {
            ...
            if self.start {
                if let Some(pow_hash) = self.pow_hash.clone() {
                    state_update_counter += self.solve(&pow_hash);
